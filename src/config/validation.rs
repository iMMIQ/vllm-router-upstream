...
mod tests {
    ...
    #[test]
    fn test_validate_roundrobin_with_vllm_pd_mode() {
        // RoundRobin with vLLM PD mode is now supported
        let config = RouterConfig::new(
            RoutingMode::VllmPrefillDecode {
    ...
    #[test]
    fn test_validate_cache_aware_with_vllm_pd_mode() {
        // CacheAware with vLLM PD mode is now supported
        let config = RouterConfig::new(
            RoutingMode::VllmPrefillDecode {
                prefill_urls: vec![("http://prefill:8000".to_string(), None)],
                decode_urls: vec!["http://decode:8000".to_string()],
    ...
    #[test]
    fn test_validate_vllm_pd_mode_with_separate_policies() {
        // Test vLLM PD mode with different policies for prefill and decode
        let config = RouterConfig::new(
            RoutingMode::VllmPrefillDecode {
                prefill_urls: vec![
    ...
    #[test]
    fn test_validate_vllm_pd_mode_power_of_two_insufficient_workers() {
